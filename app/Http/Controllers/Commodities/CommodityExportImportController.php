...
class CommodityExportImportController extends Controller
{
    public function export()
    {
        ...
        if(count($commodities)!=0)
            return Excel::download(new Export, 'daftar-arsip-' . date('d-m-Y') . '.xlsx');
        return redirect()->back()->withInput()->withErrors('Tidak ada File');
    }
    ...
    public function import()
    {
        try {
            ...
            return redirect()->back()->with('sukses', 'Import arsip Berhasil');
        } catch (\Throwable $th) {
